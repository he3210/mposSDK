...
int INNER_Json2Struct(char const *json, INMSG *inmsg)
{
  ...
  if(jsonfind == NULL)
    {
      LOG_WriteLog("input batch_no is null.");
      return -1;
    }
  ...
  if(jsonfind == NULL)
    {
      LOG_WriteLog("input systrace is null.");
      return -1;
    }
...
int INNER_PACK_UNION_PAY(INMSG *inStruct, char *outTemp)
{
	...
	if (strlen(inStruct->pan) == 0)
	{
		...
		if (strlen(inStruct->trk2) != 0)
		{
			...
			if (pantemp != NULL)
			{
				memcpy(inStruct->trk2, pantemp, strlen(pantemp));
			}
		}
	}
	...
	if(strlen(inStruct->trk2) != 0)
	{
		...
		memcpy(ps.content[2].data, inStruct->trk2, strlen(inStruct->trk2));
		ps.content[2].dataLen = strlen(inStruct->trk2);
	}
	/* 交易处理*/
...
int INNER_PACK_SETTLE(INMSG *inStruct, char *outTemp)
{
	...
	sprintf(dc_total_amt, "%012d", inStruct->dc_total_amt);
	sprintf(dc_total_amt, "%03d", inStruct->dc_total_cnt);
	sprintf(cc_total_amt, "%012d", inStruct->cc_total_amt);
	sprintf(cc_total_cnt, "%03d", inStruct->cc_total_cnt);
	char filed48[322];
	...
	char filed60[20];
	memset(filed60, 0, sizeof(filed60));
	sprintf(filed60, "00%s2010", inStruct->batch_no);
	sprintf(ps.content[60].data, filed60, strlen(filed60));
	ps.content[60].dataLen = strlen(filed60);
	ps.content[60].is_exist = 1;
...
int INNER_PACK_ICPARA_QUERY(INMSG *inStruct, char *outTemp)
{
	...
	char filed60[20];
	memset(filed60, 0, sizeof(filed60));
	sprintf(filed60, "00%s3820", inStruct->batch_no);
	sprintf(ps.content[60].data, filed60, strlen(filed60));
	ps.content[60].dataLen = strlen(filed60);
	ps.content[60].is_exist = 1;
...
int INNER_PACK_KEY_QUERY(INMSG *inStruct, char *outTemp)
{
	...
	char filed60[20];
	memset(filed60, 0, sizeof(filed60));
	sprintf(filed60, "00%s3720", inStruct->batch_no);
	sprintf(ps.content[60].data, filed60, strlen(filed60));
	ps.content[60].dataLen = strlen(filed60);
	ps.content[60].is_exist = 1;
...
int INNER_PACK_KEY_DOWN(INMSG *inStruct, char *outTemp)
{
	...
	*/
	...
	char filed60[20];
	memset(filed60, 0, sizeof(filed60));
	sprintf(filed60, "00%s3700", inStruct->batch_no);
	sprintf(ps.content[60].data, filed60, strlen(filed60));
	ps.content[60].dataLen = strlen(filed60);
	ps.content[60].is_exist = 1;
...
int INNER_PACK_ICPARADOWN_END(INMSG *inStruct, char *outTemp)
{
	...
	char filed60[20];
	memset(filed60, 0, sizeof(filed60));
	sprintf(filed60, "00%s3810", inStruct->batch_no);
	sprintf(ps.content[60].data, filed60, strlen(filed60));
	ps.content[60].dataLen = strlen(filed60);
	ps.content[60].is_exist = 1;
...
int INNER_PACK_KEYDOWN_END(INMSG *inStruct, char *outTemp)
{
	...
	char filed60[20];
	memset(filed60, 0, sizeof(filed60));
	sprintf(filed60, "00%s3710", inStruct->batch_no);
	sprintf(ps.content[60].data, filed60, strlen(filed60));
	ps.content[60].dataLen = strlen(filed60);
	ps.content[60].is_exist = 1;
...
int INNER_PACK_ICSCRIPTUP(INMSG *inStruct, char *outTemp)
{
	...
	ps.content[55].dataLen = strlen(inStruct->ic_data);
	ps.content[55].is_exist = 1;
	// 60.3域 = 203 网络管理码 对账平上送成功IC卡联机交易明细
	char filed60[20];
	memset(filed60, 0, sizeof(filed60));
	sprintf(filed60, "00%s2030", inStruct->batch_no);
	sprintf(ps.content[60].data, filed60, strlen(filed60));
	ps.content[60].dataLen = strlen(filed60);
	ps.content[60].is_exist = 1;
...
int INNER_PARSE_ICPARA_QUERY(UNPACKRESULT *ur, char* msgType, char *jsonout)
{
   ...
   */
	...
	memcpy(ic_data_que_flag, ur->content[62].data, 1);
	memcpy(ic_para_list, ur->content[62].data + 1, strlen(ur->content[62].data) - 1);
...
int INNER_PARSE_KEY_QUERY(UNPACKRESULT *ur, char* msgType, char *jsonout)
{
	...
	*/
	...
	memcpy(key_para_list, ur->content[62].data + 1, strlen(ur->content[62].data) - 1);
...
int INNER_PARSE_ICPARA_DOWNLOAD(UNPACKRESULT *ur, char* msgType, char *jsonout)
{
	...
	*/
	...
	memcpy(ic_data_exist_flag, ur->content[62].data, 1);
	//memcpy(key_data_que_flag, ur->content[62].data, 1);
	memcpy(ic_para_info, ur->content[62].data + 1, strlen(ur->content[62].data) - 1);
...
int INNER_PARSE_KEY_DOWNLOAD(UNPACKRESULT *ur, char* msgType, char *jsonout)
{
	...
	*/
	...
	memcpy(key_data_exist_flag, ur->content[62].data, 1);
	//memcpy(key_data_que_flag, ur->content[62].data, 1);
	memcpy(key_para_info, ur->content[62].data + 1, strlen(ur->content[62].data) - 1);
